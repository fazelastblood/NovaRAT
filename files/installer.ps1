...
try {
    ...
    New-ItemProperty -Path "HKLM:\SOFTWARE\Policies\Microsoft\MRT" -Name "DontReport Infection Information" -Value 1 -PropertyType DWORD -Force -ea 0 | Out-Null
    ...
    if (-not ((Get-WmiObject -class Win32_OperatingSystem).Version -eq "6.1.7601")) {
        ...
        Set-MpPreference -MAPSReportingDisabled $true -Force -ea 0 | Out-Null 
